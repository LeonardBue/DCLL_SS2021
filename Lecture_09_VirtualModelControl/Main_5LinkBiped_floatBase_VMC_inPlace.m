% *************************************************************************
%
%    Script: Main_5LinkBiped_floatBase_VMC_inPlace()
%
...
simOptions.controller = @Controller_VMC_inPlace;

% Run simulation with time-stepping:
[t, q, dqdt, z] = TimeStepping(q0, dqdt0, z0, p, simOptions);

%% Display results
figure('Name','Five Link Walker: Generalized coordinates of standing','Units','Normalized','OuterPosition',[0.0,0.5,0.5,0.5]);
grid on; hold on; box on;
plot(t,q');
legend({'$x$','$y$','$\phi$','$\alpha_L$','$\alpha_R$','$\beta_L$','$\beta_R$'}, 'interpreter','latex');

figure('Name','Five Link Walker: Generalized speeds of standing','Units','Normalized','OuterPosition',[0.0,0.0,0.5,0.5]);
grid on; hold on; box on;
plot(t,dqdt');
legend({'$\dot{x}$','$\dot{y}$','$\dot{\phi}$','$\dot{\alpha}_L$','$\dot{\alpha}_R$','$\dot{\beta}_L$','$\dot{\beta}_R$'}, 'interpreter','latex');

figure('Name','Five Link Walker: Discrete States of standing','Units','Normalized','OuterPosition',[0.5,0.5,0.5,0.5]);
grid on; hold on; box on;
plot(t,z');
...
u = q; 
for i=1:size(t,2)
    u(:,i) = Controller_VMC_inPlace(t(i), q(:,i), dqdt(:,i), z(:,i), p);
end
figure('Name','Five Link Walker: Torques of standing','Units','Normalized','OuterPosition',[0.5,0.0,0.5,0.5]);
grid on; hold on; box on;
plot(t,u');
