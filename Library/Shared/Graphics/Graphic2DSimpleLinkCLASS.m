% *************************************************************************
%
% classdef Graphic2DSimpleLinkCLASS(q, p, rFoot, rCoG) < OutputCLASS
%
% A link (segment) and CoG based graphical representation of a model of a
% legged system.  On the MATLAB search path, an implementation of the
% following functions must be found:
%
% CoGs    = CoGPositions(q, p)
% links   = LinkPositions(q, p)
% footPts = footPtPositions(q, p)
% 
% which map a vector of generalized coordinates and system parameters into
% vectors of CoG positions and orientations, link positions, and foot point
% positions, respectively.  These functions are typically autogenerated by
% the script SymbolicComputationOfEoM. The class then draws: 
%
...
%  obj = Graphic2DSimpleLinkCLASS(q, p, rFoot) with the following inputs:
%         - A vector of generalized states 'q'
...
%
%   See also OUTPUTCLASS, SYMBOLICCOMPUTATIONOFEOM, COGPPSITIONS, LINKPOSITIONS, FOOTPTPOSITIONS
...
classdef Graphic2DSimpleLinkCLASS < OutputCLASS 
    ...
    methods
        function obj = Graphic2DSimpleLinkCLASS(q, p, rFoot, rCoG)
        ...
        function obj = update(obj, q, ~, t, ~)
            ...
            CoGs    = CoGPositions(q,obj.p);
            links   = LinkPositions(q,obj.p);
            footPts = FootPtPositions(q,obj.p);
