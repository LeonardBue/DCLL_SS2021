% *************************************************************************
%
%   tau = Controller_ZMP(t, q, dqdt, z, p)
%
% This is a ZMP-planning based walking controller that uses an inverse
% kinematics approach and PID controller to let the feet and main body
% track desired trajectories that keep the ZMP within the support polygon.
%
...
function tau = Controller_ZMP(t, q, dqdt, z, p)
    ...
    % coordinates)
    % Compute the ZMP Trajectories only once and keep them in memory:
    persistent tPLAN xF_lPLAN yF_lPLAN xF_rPLAN yF_rPLAN xCoGPLAN yCoGPLAN
    if isempty(tPLAN)
        [tPLAN, xF_lPLAN, yF_lPLAN, xF_rPLAN, yF_rPLAN, xCoGPLAN, yCoGPLAN] = PlanZMPTrajectories();
    end
    % Interpolate the COG trajectories from the ZMP control:
    xF_l = interp1(tPLAN, xF_lPLAN, t);
    xF_r = interp1(tPLAN, xF_rPLAN, t);
    yF_l = interp1(tPLAN, yF_lPLAN, t);
    yF_r = interp1(tPLAN, yF_rPLAN, t);
    xM = interp1(tPLAN, xCoGPLAN, t);
    yM = interp1(tPLAN, yCoGPLAN, t); % CoG is located approximately at the hip (which is the reference for the main body)
