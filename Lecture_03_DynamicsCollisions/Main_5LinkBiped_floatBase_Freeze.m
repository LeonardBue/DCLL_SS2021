% *************************************************************************
%
%    Script: Main_5LinkBiped_floatBase_Freeze
%
% Main script for lecture_03.
%
...
y      =  1;    % [l] vertical main body position
phi    =  0;    % [rad] angle of the main body with the vertical (0 = orthog to the ground, >0 is ccw)
alphaL =  0.25; % [rad] angle of the left thigh wrt the main body (0 = parallel to mb, >0 is ccw)
alphaR =  1;    % [rad] angle of the right thigh wrt the main body (0 = parallel to mb, >0 is ccw)
betaL  = -0.5;  % [rad] angle of the left shank wrt the left thigh (0 = straight knee, >0 is ccw)
betaR  = -0.5;  % [rad] angle of the right shank wrt the right thigh (0 = straight knee, >0 is ccw)
% Velocities:
...
simOptions.controller = @Controller_Freeze;

% Run event-based simulation:
[t, q, dqdt, z] = HybridDynamics(q0, dqdt0, z0, p, simOptions);

% As an alternative, one could run the Time Stepping Algorithm for the
% simulation (not covered in class).  It uses the same syntax:
% Run simulation with time-stepping:
[t, q, dqdt, z] = TimeStepping(q0, dqdt0, z0, p, simOptions);
% This does not give exactly the same result, but this is not expected,
% as the two simlulators cover friction differently and use different
% numerical orders in their simulation.

%% Display results
figure('Name','Five Link Walker: Generalized coordinates of a non-periodic motion','Units','Normalized','OuterPosition',[0.0,0.5,0.5,0.5]);
grid on; hold on; box on;
plot(t,q');
legend({'$x$','$y$','$\phi$','$\alpha_L$','$\alpha_R$','$\beta_L$','$\beta_R$'}, 'interpreter','latex');

figure('Name','Five Link Walker: Generalized speeds of a non-periodic motion','Units','Normalized','OuterPosition',[0.0,0.0,0.5,0.5]);
grid on; hold on; box on;
plot(t,dqdt');
legend({'$\dot{x}$','$\dot{y}$','$\dot{\phi}$','$\dot{\alpha}_L$','$\dot{\alpha}_R$','$\dot{\beta}_L$','$\dot{\beta}_R$'}, 'interpreter','latex');

figure('Name','Five Link Walker: Discrete States of a non-periodic motion','Units','Normalized','OuterPosition',[0.5,0.5,0.5,0.5]);
grid on; hold on; box on;
plot(t,z');
...
u = q; 
for i=1:size(t,2)
    u(:,i) = Controller_Freeze(t(i), q(:,i), dqdt(:,i), z(:,i), p);
end
figure('Name','Five Link Walker: Torques of a non-periodic motion','Units','Normalized','OuterPosition',[0.5,0.0,0.5,0.5]);
grid on; hold on; box on;
plot(t,u');
